document.addEventListener("DOMContentLoaded", async () => {
    // Obtener parámetros 'curso' y 'nivel' desde la URL
    const urlParams = new URLSearchParams(window.location.search);
    const curso = "cinema"; // Curso predeterminado para este apartado
    const nivel = urlParams.get("nivel"); // Nivel: 'basico', 'intermedio', 'avanzado'
  
    if (!nivel) {
      console.error("Falta el parámetro 'nivel' en la URL.");
      return;
    }
        ...
        datosCurso.recursos.forEach((recurso) => {
          const recursoHTML = `
            <div class="content-card">
              <img src="/public/img/ninos/bannerCine.jpg" alt="${recurso.titulo}">
